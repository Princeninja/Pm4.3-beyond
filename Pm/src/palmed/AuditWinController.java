...
public class AuditWinController extends GenericForwardComposer {

	private Listbox auditListbox;		// autowired
...
*/
		...
		if ( auditListbox == null ) return;
		
		// remove all items
		ZkTools.listboxClear( auditListbox );
		...
		// populate list
		AuditLog.fillListbox( auditListbox, month, year, /*userRec*/ null, /*ptRec*/ null, /*action*/ null );
	...
	public void onClick$btnPrint( Event ev ){
		...
		for( int i = 0; i < auditListbox.getItemCount(); ++i ){
			Listitem li = auditListbox.getItemAtIndex( i );
