...
public class LabBatWinController extends GenericForwardComposer {

	private Listbox labBatListbox;	// autowired
	private Radio r_active;				// autowired
	private Radio r_inactive;			// autowired
	private Radio r_all;				// autowired
	private Window labBatWin;		// autowired
	...
	public void refreshList( String searchString ){
		...
		if ( labBatListbox == null ) return;
		
		// remove all items
		ZkTools.listboxClear( labBatListbox );
	...
	public void onClick$btnEdit( Event ev ){
		...
		if ( labBatListbox.getSelectedCount() < 1 ){
			try { Messagebox.show( "No item selected." ); } catch (Exception e) { /*ignore*/ }
			return;
		}

		// get selected item's rec
		Rec rec = (Rec) labBatListbox.getSelectedItem().getValue();
		if ( ! Rec.isValid( rec )) return;

		// call edit routine
		if ( LabBatEdit.edit( rec, labBatWin )){
			refreshList();
		}		
		return;
	}
