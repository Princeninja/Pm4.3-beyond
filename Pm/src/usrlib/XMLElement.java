...
*****************************************************************************/
// 2010-12-20 jrp - added getChildByName() and getElementByPathName() methods
// 2011-04-06 jrp - setName() - ignore prefix
// 2016-08-30 prince - added getChildByNumber() method
...
// JRP
// My changes:
// Make ignoreCase false by default
// 2011-04-06 jrp - setName() - ignore everything before and up through a colon. (ignore prefix)
// 2011 jrp - add toFile() 
// 2011 jrp - had toFile() and toString() write an XML header
...
public class XMLElement
{
   ...
   public XMLElement()
   {
       this(new Hashtable(), false, true, /*ignore case*/ false );
   }
   ...
   {
       this(entities, false, true, /*ignore case*/ false );
   }
   ...
   {
       this(new Hashtable(), skipLeadingWhitespace, true, /*ignore case*/ false );
   }
   ...
   {
       this(entities, skipLeadingWhitespace, true, /*ignore case*/ false );
   }
   ...
   {
	   if (( name.indexOf( ':' ) >= 0 ) && ( name.length() > ( name.indexOf( ':' ) +1 ))){
   ...
   /**
    * Writes the XML element to a file.
    *
   ...
   public boolean toFile( String filename )
   {
       try {
          // ByteArrayOutputStream out = new ByteArrayOutputStream();
           ...
           // jrp - I added this - (couldn't figure out where to add it)
   ...
   /**
    * Sets the ignore case flag (so doesn't have to be done in constructor call).
    *
    * @param ignoreCase
    *
    */
   ...
   public XMLElement getChildByName( String name ){
	   ...
	   Enumeration<XMLElement> elist = this.enumerateChildren();
	   while ( elist.hasMoreElements()){
		   XMLElement e2 = (XMLElement) elist.nextElement();
   ...
   public XMLElement getElementByPathName( String pathname ){
	   
	   String[] a = pathname.split( "/" );
	   XMLElement e = this;
	   int i = 0;
	   
	   // is first token empty or null?  If so, skip.
	   if (( a[i] == null ) || ( a[i].length() == 0 )) ++i;
	   
	   // is first token in path the root element?  If so, skip.
	   if ( e.getName().equalsIgnoreCase( a[i])) ++i;
	   
	   for ( ; i < a.length; ++i ){
		   
		   //System.out.println( "child=" + a[i] );
		   e = e.getChildByName( a[i] );
		   if ( e == null ){
			   //System.out.println( "child not found=" + a[i] );
			   break;
		   }
	   }
   ...
   public XMLElement getChildByNumber( int number) throws ArrayIndexOutOfBoundsException {
	   	
			return (XMLElement) this.children.elementAt(number);
